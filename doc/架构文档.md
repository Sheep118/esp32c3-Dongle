...
- **BLE 广播扫描**：持续扫描周围 BLE 设备的广播包，非阻塞周期扫描
- **BLE 模拟广播**：按用户配置构建自定义广播包并发送（可配置 MAC、间隔、数据、功率、信道）
- **白名单过滤**：支持按 MAC 地址、设备名称、厂商 ID 三种规则过滤
- **自定义输出格式**：扫描结果的串口输出格式完全可配置（高级选项），支持 POSIX 位置参数保证任意占位符顺序
- **Wi-Fi 热点配置**：开机按按键进入 AP 模式，通过 Web 页面管理全部配置
- **配置导入/导出**：Web 页面支持 JSON 配置文件导出备份和导入恢复
...
│   ├── raw_ble_advertiser.h/cpp # BLE 模拟广播模块
...
└── doc/                      # 项目文档
    ...
    └── web_config_backup.html # Web 配置页面备份
```
...
```
BleScanFormat
├── scanStartFmt  → 扫描开始时输出格式 (如 "$SCAN_START|DURATION:<duration>")
├── scanResultFmt → 扫描结果行输出格式 (如 "$BLE|MAC:<mac>|RSSI:<rssi>|...")
└── scanEndFmt    → 扫描结束时输出格式 (如 "$SCAN_END|TOTAL:<total>|MATCHED:<matched>")
```

**BLE 广播参数**：参见第 4.7 节 `BleAdvConfig`。
...
用户可在 Web 配置页"高级选项"中自定义三行输出格式（使用友好占位符）：

| 行类型 | 默认格式 | 允许的占位符 |
|--------|---------|------------|
| 扫描开始 | `$SCAN_START\|DURATION:<duration>` | `<duration>` |
| 扫描结果 | `$BLE\|MAC:<mac>\|RSSI:<rssi>\|ADDR:<addr>\|NAME:<name>\|MANUF:<manuf>\|UUID:<uuid>` | `<mac>` `<rssi>` `<addr>` `<name>` `<manuf>` `<uuid>` |
| 扫描结束 | `$SCAN_END\|TOTAL:<total>\|MATCHED:<matched>` | `<total>` `<matched>` |

**工作原理**：
1. 用户在 Web 页面输入含占位符的格式字符串（如 `<addr>_<mac>`）
2. 保存到 `ble_dongle.json` 的 `scanFormat` 字段
3. 下次开机时 `BleScanner::compileFormats()` 将占位符编译为 POSIX 位置参数（`<mac>` → `%1$s`、`<addr>` → `%3$s`）
4. 之后所有 `snprintf` 直接使用编译后的格式串

POSIX 位置参数（`%n$s`）确保用户任意调换占位符顺序时参数映射不乱。

#### 空格式处理
- 某行格式为空字符串 → 该行在扫描时不输出任何内容
- 扫描结果为空时 Web 端会弹出确认框提醒用户
...
| 功能 | 说明 |
|------|------|
| 模式切换 | 扫描器 / 模拟广播 / 串口透传（预留） |
| 白名单管理 | CRUD + 拖拽排序 |
| 扫描参数 | 间隔、窗口、类型、重复过滤 |
| 广播参数 | MAC、间隔、功率、信道、AD 数据可视化构建 |
| 高级选项 | 自定义输出格式（含占位符校验） |
| 配置导入/导出 | JSON 文件下载/上传 |
| Captive Portal | DNS 劫持实现，兼容 Android/iOS/Windows |

---

### 4.7 BLE 模拟广播器 `raw_ble_advertiser.h` / `raw_ble_advertiser.cpp`

**设计特点**：
- 基于 ESP32 BLE Arduino 库的底层 API 直接构建广播/扫描响应数据
- 支持自定义 MAC 地址、发射功率、广播间隔、信道选择
- 支持三种广播类型：`ADV_IND`（可连接）、`ADV_SCAN_IND`、`ADV_NONCONN_IND`
- 广播数据通过 Web 页面可视化构建或原始 HEX 输入

#### 配置参数

```
BleAdvConfig
├── customMac        → 自定义 MAC 地址 (空=使用默认)
├── advDataHex       → 广播数据 HEX 字符串
├── scanRespHex      → 扫描响应数据 HEX 字符串
├── txPower          → 发射功率 (-12~+9 dBm)
├── advIntervalMin   → 最小广播间隔 (×0.625ms)
├── advIntervalMax   → 最大广播间隔 (×0.625ms)
├── advDuration      → 广播时长 (秒, 0=持续)
├── advType          → 广播类型 (0/2/3)
└── channelMap       → 广播信道位掩码 (bit0=CH37, bit1=CH38, bit2=CH39)
```

---

### 4.8 嵌入脚本 `tools/embed_html.py`
